...
function applyFormType() 
{
    ...
    // Подписи полей одинаковы для обеих форм
...
{
    // Обновляем текущую форму из radio buttons
    let radios = document.getElementsByName('formType');
    for (let i = 0; i < radios.length; i++)
    {
        if (radios[i].checked)
        {
            currentFormType = radios[i].value;
            break;
        }
    }
...
{
    // Очищаем только поля для ввода комплексного числа
    form.r.value = '';
    form.phi.value = '';
    
    // Очищаем сообщения об ошибках для полей ввода
    let errorR = document.getElementById('errorR');
    if (errorR) errorR.textContent = '';
    
    let errorPhi = document.getElementById('errorPhi');
    if (errorPhi) errorPhi.textContent = '';
    
    // Убираем классы ошибок с полей ввода
    let rInput = document.getElementsByName('r')[0];
    if (rInput) rInput.classList.remove('error');
    
    let phiInput = document.getElementsByName('phi')[0];
    if (phiInput) phiInput.classList.remove('error');
