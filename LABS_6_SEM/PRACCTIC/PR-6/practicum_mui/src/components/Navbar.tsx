...
const menuLinks = [
  { label: 'Самые длинные мосты', active: true },
  { label: 'Список мостов', active: false },
  { label: 'Амурский мост', active: false },
];
            ...
            {menuLinks.map((item) => (
              <Button
                ...
                sx={{
                  ...
                  px: '25px',
                  py: '12px',
                  ...(item.active ? activeTabSx : { color: '#333' }),
                }}
              >
                {item.label}
              </Button>
          ...
          {menuLinks.map((item) => (
            ...
            >
              <ListItemText
                primary={item.label}
                slotProps={{
                  primary: { sx: { fontWeight: 'bold', ...(item.active ? { color: '#fff' } : {}) } },
                }}
              />
            </ListItemButton>
          ))}
